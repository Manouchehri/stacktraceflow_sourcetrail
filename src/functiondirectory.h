...
public:
    ...
    class SearchIterator: public boost::iterator_facade<
            SearchIterator,
            FuncCall*,
            boost::forward_traversal_tag,
            FuncCall*> {
    private:
        using EntryIterators =
            std::vector<
                std::pair<
                    FunctionDirectory::Entry::ActiveCallsIterator,
                    FunctionDirectory::Entry::ActiveCallsIterator>>;
    public:
        SearchIterator() {}
        SearchIterator(EntryIterators &&_iterators)
            : iterators(std::move(_iterators))
        {}
        static const SearchIterator end;
    private:
        friend class boost::iterator_core_access;
        size_t earliestCallPos() const {
            assert(!iterators.empty());
            auto earliestCallIterator = std::min_element(iterators.begin(), iterators.end(),
                [] (const auto &pairA, const auto &pairB) {
                    assert(*pairA.first != nullptr);
                    assert(*pairB.first != nullptr);
                    return (*pairA.first)->getSeqNum() < (*pairB.first)->getSeqNum();
                });
            return earliestCallIterator - iterators.begin();
        }
        FuncCall *dereference() const {
            assert(!iterators.empty());
            return *iterators[earliestCallPos()].first;
        }
        bool equal(const SearchIterator &other) const {
            return this->iterators == other.iterators;
        }
        void increment() {
            assert(!iterators.empty());
            size_t incrementPos = earliestCallPos();
            iterators[incrementPos].first++;
            if (iterators[incrementPos].first == iterators[incrementPos].second) {
                std::swap(iterators[incrementPos], iterators.back());
                iterators.resize(iterators.size() - 1);
            }
        }
        EntryIterators iterators;
    };

    std::pair<SearchIterator, SearchIterator> search(const std::string &pattern) {
        std::vector<std::pair<Entry::ActiveCallsIterator, Entry::ActiveCallsIterator>>
            entryIterators;
        for (Entry &entry: functions) {
            if (entry.matches(pattern) &&
                    entry.activeCallsBegin() != entry.activeCallsEnd()) {
                entryIterators.push_back(std::make_pair(
                    entry.activeCallsBegin(), entry.activeCallsEnd()));
            }
        }
        return std::make_pair(
            SearchIterator(std::move(entryIterators)),
            SearchIterator::end
        );
    }
